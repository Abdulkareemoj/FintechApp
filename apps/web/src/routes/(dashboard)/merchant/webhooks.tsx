...
const webhooks = [
  {
    ...
    recentDeliveries: [
      { id: "d1", event: "payment.completed", status: "success", time: "14:32" },
      { id: "d2", event: "payment.completed", status: "success", time: "14:28" },
  ...
  },
  {
    ...
    recentDeliveries: [
      { id: "d4", event: "payment.completed", status: "success", time: "13:45" },
      { id: "d5", event: "payment.completed", status: "success", time: "13:30" },
    ],
  },
  {
    id: "3",
...
const availableEvents = [
  ...
  { category: "Refunds", events: ["refund.created", "refund.completed", "refund.failed"] },
  { category: "Settlements", events: ["settlement.created", "settlement.completed"] },
  { category: "Disputes", events: ["dispute.created", "dispute.won", "dispute.lost"] },
];
  ...
  return (
    <MerchantLayout>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        {[
          { label: "Active Webhooks", value: "2", icon: Webhook, color: "violet" },
          { label: "Events Today", value: "1,234", icon: Code2, color: "emerald" },
          { label: "Success Rate", value: "99.2%", icon: CheckCircle2, color: "emerald" },
          { label: "Failed Deliveries", value: "3", icon: AlertTriangle, color: "amber" },
        ].map((stat, index) => (
          <motion.div
            key={stat.label}
        ...
        <Card className="bg-card/50 backdrop-blur-sm border-border/50">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>Manage your webhook endpoints and subscribed events.</CardDescription>
            </div>
            <Dialog>
              ...
              <DialogContent className="bg-card border-border max-w-lg">
                ...
                <div className="space-y-4 py-4">
                  <div className="space-y-2">
                    ...
                    <Input placeholder="https://your-domain.com/webhooks" className="bg-muted/50" />
                  </div>
                  <div className="space-y-2">
                    <Label>Description (optional)</Label>
                    <Input placeholder="e.g., Production payment notifications" className="bg-muted/50" />
                  </div>
                ...
                <DialogFooter>
                  ...
                  <Button className="bg-violet-500 hover:bg-violet-600">Add Endpoint</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
          </CardHeader>
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className={`h-10 w-10 rounded-lg flex items-center justify-center ${webhook.status === "active" ? "bg-emerald-500/10" : "bg-muted"}`}>
                      <Webhook className={`h-5 w-5 ${webhook.status === "active" ? "text-emerald-400" : "text-muted-foreground"}`} />
                    </div>
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <Badge variant="outline" className={webhook.status === "active" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : "bg-muted text-muted-foreground"}>
                          {webhook.status}
                        </Badge>
